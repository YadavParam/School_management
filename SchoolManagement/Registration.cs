    ...
    public partial class Registration : Form
    {
        ...
        private void SchoolRegister_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    loadingImage.Visible = false;
                    MessageBox.Show("Please School Name and it should be more than 2 in length and check your mobile no. is correct is it in length of 10","Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                {
                    loadingImage.Visible = false;
                    MessageBox.Show(messages_register.Message, "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                ...
                else
                {
                    if (!Db.SchoolDetails.Any(x => x.Email == Email.Text && x.Phone == PhoneNumber.Text && x.IsActive == true))
                    {
                        ...
                        {
                            loadingImage.Visible = false;
                            MessageBox.Show("Already user is created", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                    }
                    else
                    {
                        loadingImage.Visible = false;
                        MessageBox.Show("Already school is registered", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                }
            }
        ...
        {
            frm = (Login)Application.OpenForms["Login"];
            if (frm == null)
            {
                frm = new Login();
            }

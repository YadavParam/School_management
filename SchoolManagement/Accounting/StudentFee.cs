            ...
            try
            {
                ...
                if (!classFessExist)
                {
                    foreach (DataGridViewRow row in StudentFeeDataGridView.Rows)
                    {
                        ...
                        {
                            int rowIndex = row.Index + 1;
                            MessageBox.Show("Please enter valid numeric values for Yearly Fees in row " + rowIndex, "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                    }
